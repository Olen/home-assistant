...
async def _get_plantbook_token(hass, client_id=None, secret=None):
    ...
    try:
        ...
        _LOGGER.error("Unable to get token from plantbook API: %s", e)
...
class Plant(Entity):
    ...
    async def async_added_to_hass(self):
        ...
        if PLANTBOOK_TOKEN and self._species:
            _LOGGER.debug(
                f"Getting plantbook-data for {self.name} {self._species}"
            )
    ...
    async def _get_plantbook_data(self):
        ...
        if not PLANTBOOK_TOKEN:
            _LOGGER.debug(f"No plantbook token for {self.name}")
            return
        url = f"https://open.plantbook.io/api/v1/plant/detail/{self._species}/"
        headers = {"Authorization": f"Bearer {PLANTBOOK_TOKEN}"}
        ...
        try:
            ...
            _LOGGER.error(f"Unable to get plant data from plantbook API: {e}")

    def _set_conf_value(self, var, val):
        """Ensures that values explicitly set in the config is not overwritten"""
        _LOGGER.debug(
            "Plant: {} Var: {} Config: {} Value: {}".format(
                self.name, var, self._config.get(var), val
            )
        )
